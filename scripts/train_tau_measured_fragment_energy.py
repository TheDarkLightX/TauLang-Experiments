#!/usr/bin/env python3
"""Train TauEnergy from measured route labels, not a synthetic route oracle."""
...
from tau_energy.fragment import (
    ...
    measured_fragment_training_summary,
...
def main() -> int:
    ...
    parser.add_argument("--out", type=Path, default=Path("results/local/tau-energy/measured_fragment_training_report.json"))
    parser.add_argument("--verify", type=Path)
    args = parser.parse_args()

    if args.verify:
        data = json.loads(args.verify.read_text(encoding="utf-8"))
        ok = verify_measured_fragment_training_report(data)
        print(json.dumps({"ok": ok, "summary": measured_fragment_training_summary(data)}, indent=2, sort_keys=True))
    ...
    print(json.dumps({"out": args.out.as_posix(), **measured_fragment_training_summary(report)}, indent=2, sort_keys=True))
    return 0 if verify_measured_fragment_training_report(report) else 1
